...
class Mars : RenderableEntity, EntityMouseDragHandler {
    
    var fillStyle = FillStyle(color:Color(.brown))
    var strokeStyle = StrokeStyle(color:Color(.black))
    let circle = Ellipse(center:Point(), radiusX:15, radiusY:15, fillMode:.fillAndStroke)
    var radians = 0.0
    ...
    override func calculate(canvasSize:Size) {
        ...
        circle.center = Point(x:sun.circle.center.x + (sun.circle.radiusX * 6) + (circle.radiusX * 3), y:sun.circle.center.y) 
