...
class Venus : RenderableEntity, EntityMouseDragHandler {
    
    var fillStyle = FillStyle(color:Color(.orange))
    var strokeStyle = StrokeStyle(color:Color(.black))
    let circle = Ellipse(center:Point(), radiusX:20, radiusY:20, fillMode:.fillAndStroke)
    var radians = 0.0
  
    init(){
        super.init(name:"Venus")
    ...
    override func calculate(canvasSize:Size) {
        ...
        circle.center = Point(x:sun.circle.center.x + (sun.circle.radiusX * 3) + (circle.radiusX * 2), y:sun.circle.center.y) 
